...
function compute_happiness_discrete(model.social, which_agent)
...
function compute_happiness_continuous(model.social, which_agent)
    neigh = Graphs.neighbors(model.social, which_agent)
    count_neighbours_same_group = 0
    for i in neigh
        count_neighbours += 1
...
function is_happy(happiness, segregation_threshold)
    happiness > segregation_threshold
end

function probability_add_edge(which_agent, friending_probability)
    new_prob_friend = rand(1:total_agents)
